module Types
  class FreeAgencyPeriodType < Types::BaseObject
    ...
    field :bids, [Types::BidType], null: true do
      ...
      argument :leading, Boolean, required: false
    end
    ...
    def bids(team_id: nil, active:, leading: nil)
      query = Bid.where(free_agency_period_id: object.id, is_active: active)
      query = query.where(team_id: team_id) if team_id
      query = query.where(is_leading: leading) if !leading.nil?
      query
    end
